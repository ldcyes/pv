...
def regression(df_org,features_remain,features_x,regress_start_date,regression_train_targtes,change,regression_log,back_time):
     ...
     file = open('profilo_inference'+str(stock)+str(reg_inc_pcent[str(regression_train_targtes[0])])+str(regression_train_targtes)+str(change)+'.csv', 'w', newline='')
     ...
     for date in range(regress_start_date,df_org.shape[0]+1,1):
          ...
          train,test = train_test_split(filtered_df_trainning[features_remain][0:-1],test_size=test_size,shuffle=True)
          ...
          print("train start date",filtered_df_trainning.iloc[0][y_stock+'date'])
          #print(filtered_df[features_remain][0:1])
          print("train end date",filtered_df_trainning.iloc[-2][y_stock+'date'])     
          #print(filtered_df[features_remain][-2:-1])
          print("test start date",filtered_df_inference.iloc[-1][y_stock+'date'])
          ...
          if(is_first_price):
               ...
               is_first_price = 0
               cur_position = int(start_value/cur_price)
               cur_free = start_value - cur_position*cur_price
          ...
          for target in regression_train_targtes:
               ...
               for model in model_list:
                    ...
                    print(filtered_df_inference[-1:][y_stock+'date'])
                    if(sel_model==1):
               ...
               writer.writerow([filtered_df_inference[-1:][y_stock+'date'],
                                cur_price,
                                cur_free+cur_position*cur_price,
                                cur_price*start_value/first_price,
                                buy_condition,
                                buy_position,
                                sell_condition,
                                sell_position,
